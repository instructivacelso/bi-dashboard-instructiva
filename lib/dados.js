...
import { FORMULARIOS, FORMULARIOS_COMERCIAL, formularioAplicavel, lancamentosDoFormulario } from './formularios.js';
...
const CAMPOS_LANC = `l.id, l.nome, l.status, l.data_live, l.live_liberada, l.product_id, l.gerente_id,
  ...
  l.cpl_max, l.cac_max, l.roas_min, l.orcamento, l.permanente, p.nome AS produto`;
...
export async function lancamentosDoUsuario(u) {
  ...
  const atribuidos = await q(
    `SELECT DISTINCT ${CAMPOS_LANC} FROM launches l
      ...
      WHERE a.user_id=$1 OR l.gerente_id=$1 OR l.permanente
      ORDER BY l.id DESC`,
    [u.id]
  );
  if (atribuidos.some((l) => !l.permanente)) return atribuidos;
...
export async function tarefasDoDia(u, hoje) {
  ...
  for (const form of forms) {
    const fc = FORMULARIOS_COMERCIAL[form];
    if (fc) {
      const r = await q1(`SELECT id, updated_at FROM ${fc.tabela} WHERE user_id=$1 AND data_ref=$2`, [u.id, hoje]);
      tarefas.push({ form, titulo: fc.titulo, descricao: fc.descricao, href: fc.rota, lancamento: null, concluido: !!r, registroId: r?.id || null, ultimoEnvio: r?.updated_at || null, detalhe: null });
      continue;
    }
    const f = FORMULARIOS[form];
    if (!f) continue;
    const aplicaveis = lancamentosDoFormulario(form, lancs, hoje);
...
export async function pendenciasEquipe(hoje, setor = 'marketing') {
  ...
  for (const { id } of ids) {
    ...
    for (const t of tarefas) if (!t.aguardando && t.lancamento) lista.push({ usuario: u.nome, usuarioId: u.id, ...t });
  }
  return lista;
}
