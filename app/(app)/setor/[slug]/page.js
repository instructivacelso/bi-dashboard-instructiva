...
export default async function Setor(props) {
  ...
  const visiveis = veDashboardSetor(u, setor.slug) || cidadesVisiveis(u).includes(setor.slug) ? subs : subs.filter((s) => meus.includes(s.formulario));
      ...
      {visiveis.length === 0 ? <Vazio>Nenhum formulário atribuído a você neste setor.</Vazio> : (
        <div className="grade g3">
          {visiveis.map((s) => {
            ...
            return (
              <a key={s.id} href={FORMULARIOS_COMERCIAL[s.formulario] ? rotaFormulario(s.formulario) : `/marketing/${s.formulario}${pend?.lancamento ? `?lancamento=${pend.lancamento.id}` : ''}`} className="painel atividade" style={{ color: 'inherit', textDecoration: 'none' }}>
